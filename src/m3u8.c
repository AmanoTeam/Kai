...
int m3u8playlist_load_file(
	...
	err = m3u8_set_uri(playlist, M3U8_BASE_URI_TYPE_LOCAL_FILE, absolute_path);
...
int m3u8playlist_load_url(
	...
	err = m3u8_set_uri(playlist, M3U8_BASE_URI_TYPE_URL, url);
...
static enum M3U8BaseURIType m3u8baseuri_guess_type(const char* const something) {
	...
	if (status) {
		return M3U8_BASE_URI_TYPE_LOCAL_FILE;
	}
	
	return M3U8_BASE_URI_TYPE_TEXT;
...
int m3u8playlist_load(
	...
	const enum M3U8BaseURIType type = m3u8baseuri_guess_type(something);
