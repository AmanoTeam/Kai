...
int m3u8stream_load_subresource(
	...
	err = m3u8playlist_load_subresource(&root->playlist, &subresource->playlist, something);
	
	if (err != M3U8ERR_SUCCESS) {
		return err;
	}
	
	err = m3u8stream_parse(subresource);
...
static int m3u8download_addqeue(
	...
	switch (subresource->playlist.uri.type) {
		case M3U8_BASE_URI_TYPE_URL:
			err = m3u8uri_resolve_url(subresource->playlist.uri.uri, uri, &resolved_uri);
			break;
		case M3U8_BASE_URI_TYPE_LOCAL_FILE:
			err = m3u8uri_resolve_path(subresource->playlist.uri.uri, uri, &resolved_uri);
			break;
		default:
	...
	*/
	if (subresource->playlist.uri.type == M3U8_BASE_URI_TYPE_LOCAL_FILE) {
...
int m3u8stream_download(
	...
	qeue.size = sizeof(*qeue.items) * subresource->offset;
	...
	for (index = 0; index < subresource->offset; index++) {
		struct M3U8StreamItem* const item = &subresource->items[index];
	...
	do {
		status = m3u8playlist_ideltag(&subresource->playlist, M3U8_TAG_EXT_X_BYTERANGE);
	} while (status);
	
	tag = m3u8playlist_igettag(&subresource->playlist, M3U8_TAG_EXT_X_MAP);
