...
int main(int argc, argv_t* argv[]) {
	...
	if (options.output != NULL && options.show_formats) {
	...
	if (!options.show_formats && options.output == NULL) {
	...
	err = m3u8stream_load(&stream, options.url);
	...
	output_stream = fstream_open(options.output, FSTREAM_WRITE);
